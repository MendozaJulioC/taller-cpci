// src/components/ui/ContadorInscritos.jsx
"use client";
...
export default function ContadorInscritos() {
  ...
  // Lista de correos autorizados para ver la lista
  ...
  return (
    <>
      ...
      <div 
        className={`fixed bottom-6 right-6 z-50 group ${usuarioAutorizado ? 'cursor-pointer' : 'cursor-default'}`}
        onClick={handleClick}
        title={usuarioAutorizado ? 'Click para ver lista de participantes' : ''}
      >
        <div
          className={`
            relative
            ...
            ${usuarioAutorizado ? 'hover:scale-105 hover:shadow-2xl hover:shadow-slate-300/50 hover:border-slate-300' : ''}
        ...
        <div className="absolute -top-12 right-0 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0 pointer-events-none">
          <div className="bg-slate-800/90 backdrop-blur-sm text-white text-[10px] font-medium px-3 py-1.5 rounded-lg whitespace-nowrap shadow-lg">
            <span className="flex items-center gap-1.5">
              ...
              {usuarioAutorizado ? 'Click para ver lista de participantes' : 'Actualizado en tiempo real'}
            </span>
          </div>
